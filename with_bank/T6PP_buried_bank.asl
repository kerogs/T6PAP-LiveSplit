...
startup
{
    ...
    {
        {1, Tuple.Create("paralyzer", "Take Paralyzer")},
        {2, Tuple.Create("bank", "Visit Bank")},
        {3, Tuple.Create("power_on", "Turn On Power")},
        {4, Tuple.Create("mansion", "Enter Mansion")},
        {5, Tuple.Create("pack_a_punch", "Pack-A-Punch Weapon")}
    };
    ...
    vars.filePath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
        "Plutonium", "storage", "t6", "raw", "scriptdata", "timer"
    );
}
